...
export const CREATOR_INTEREST_ROLE_OPTIONS = [
  ...
  [
    ...
    "Community, collaboration, or other mission interest",
  ],
] as const;
...
export function validateCreatorInterestForm(form: CreatorInterestFormState) {
  ...
  if (form.motivation.trim().length < 10)
    errors.motivation = "Please tell us a little more about your interest.";
  if (form.participationPreferences.trim().length < 10)
    errors.participationPreferences =
      "Please describe how you would like to participate.";
  if (form.roleCodes.length === 0)
    errors.roleCodes = "Select at least one interest.";
  if (!form.willingToShare)
    errors.willingToShare = "Choose a story-sharing preference.";
  if (!form.comfortLevel)
    errors.comfortLevel = "Choose a participation comfort level.";
  if (!form.consent)
    errors.consent = "Consent is required before we can contact you.";
  if (form.socials.length > 10)
    errors.socials = "Add no more than 10 social profiles.";
  ...
  form.socials.forEach((social, index) => {
    ...
    if (social.followerCount.trim()) {
      ...
      if (
        ...
        followers < 0 ||
        followers > 2_147_483_647
      ) {
        errors[`${prefix}.followerCount`] =
          "Follower count must be a whole number from 0 to 2,147,483,647.";
      }
    }
  });
...
export function buildCreatorInterestPayload(
  ...
  return {
    ...
    ...(trimOrUndefined(form.preferredName) && {
    ...
    ...(trimOrUndefined(form.veteranConnection) && {
      veteran_connection: form.veteranConnection.trim(),
    }),
    ...
    ...(trimOrUndefined(form.personalMission) && {
      personal_mission: form.personalMission.trim(),
    }),
    ...(trimOrUndefined(form.fundraisingGoal) && {
      fundraising_goal: form.fundraisingGoal.trim(),
    }),
    ...(trimOrUndefined(form.additionalInformation) && {
      additional_info: form.additionalInformation.trim(),
    }),
    consent: form.consent,
    social_profiles: form.socials.map((social) => ({
      platform: social.platform,
      ...(trimOrUndefined(social.handle) && { handle: social.handle.trim() }),
      ...(trimOrUndefined(social.profileUrl) && {
        profile_url: social.profileUrl.trim(),
      }),
      ...(social.followerCount.trim() && {
        follower_count: Number(social.followerCount),
      }),
    })),
...
export function UnifiedBtyForm({ initialLane }: { initialLane?: InitialLane }) {
  ...
  useEffect(() => {
    const preset: RoleCode[] =
      initialLane === "share-story" ? ["bty_promoter"] : [];
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (
      ...
      PUBLIC_PAYLOAD_CLIENT_LIMIT_BYTES
    ) {
    ...
    return (
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-xl text-muted-foreground">
          A ValorWell team member will review it and follow up when there is a
          useful next step.
        </p>
      </div>
    );
  ...
  return (
    <form
      noValidate
      onSubmit={submit}
      className="space-y-8 rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6 md:p-8"
    >
      <div>
        <h3 className="text-2xl font-bold text-foreground">
          Beyond The Yellow guest interest
        </h3>
        <p className="mt-2 text-sm text-muted-foreground">
          This form does not create an account. Share only what ValorWell needs
          to understand how you would like to participate.
        </p>
      </div>
      ...
      {Object.keys(errors).length > 0 && (
        ...
        >
          ...
          {errors.payload && (
            <p className="mt-1 font-medium">{errors.payload}</p>
          )}
        </div>
      )}
      {status === "error" && (
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            aria-describedby={errors.state ? "interest-state-error" : undefined}
          ...
          {errors.state && (
            <p id="interest-state-error" className={errorClass}>
              {errors.state}
            </p>
          )}
        </div>
      ...
      <fieldset
        className="space-y-4"
        aria-describedby={errors.roleCodes ? "interest-roles-error" : undefined}
      >
        <legend className="text-lg font-semibold text-foreground">
          How would you like to participate?
        </legend>
        <p className="text-sm text-muted-foreground">
          Select every option that applies. These choices help route follow-up;
          they do not create a partnership or account.
        </p>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {CREATOR_INTEREST_ROLE_OPTIONS.map(([value, label]) => (
            ...
            >
              <input
                type="checkbox"
                checked={form.roleCodes.includes(value)}
        ...
        {errors.roleCodes && (
          <p id="interest-roles-error" className={errorClass}>
            {errors.roleCodes}
          </p>
        )}
      </fieldset>
      ...
      <div className="grid gap-5">
        <div>
          <label
            htmlFor="interest-veteran-affiliation"
            className="block text-sm font-medium text-foreground"
          >
            Veteran or military-family affiliation
          </label>
          <select
            id="interest-veteran-affiliation"
            value={form.veteranAffiliation}
            onChange={(event) =>
              update(
                "veteranAffiliation",
                event.target
                  .value as CreatorInterestFormState["veteranAffiliation"],
              )
            }
            className={inputClass}
          >
            ...
            <option value="family_member">
              Military or veteran family member
            </option>
            <option value="military_connected">
              Active duty, Guard, Reserve, caregiver, or otherwise
              military-connected
            </option>
        ...
        <TextAreaField
          id="interest-veteran-connection"
          label="Veteran or military-family connection (optional)"
          value={form.veteranConnection}
          onChange={(value) => update("veteranConnection", value)}
          maxLength={1000}
        />
        <TextAreaField
          id="interest-motivation"
          label="Why are you interested in ValorWell or Beyond The Yellow?"
          required
        ...
        />
        <TextAreaField
          id="interest-participation"
          label="How would you like to participate or help?"
          required
          value={form.participationPreferences}
          error={errors.participationPreferences}
          onChange={(value) => update("participationPreferences", value)}
          maxLength={4000}
        />
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label
            htmlFor="interest-willing-to-share"
            className="block text-sm font-medium text-foreground"
          >
          ...
          <select
            id="interest-willing-to-share"
            required
            value={form.willingToShare}
            aria-invalid={Boolean(errors.willingToShare)}
            aria-describedby={
              errors.willingToShare
                ? "interest-willing-to-share-error"
                : undefined
            }
            onChange={(event) =>
              update(
                "willingToShare",
                event.target
                  .value as CreatorInterestFormState["willingToShare"],
              )
            }
            className={inputClass}
          >
          ...
          {errors.willingToShare && (
            <p id="interest-willing-to-share-error" className={errorClass}>
              {errors.willingToShare}
            </p>
          )}
        </div>
        <div>
          ...
          <select
            ...
            aria-invalid={Boolean(errors.comfortLevel)}
            aria-describedby={
              errors.comfortLevel ? "interest-comfort-error" : undefined
            }
            onChange={(event) =>
              update(
                "comfortLevel",
                event.target.value as CreatorInterestFormState["comfortLevel"],
              )
            }
            className={inputClass}
          >
            <option value="">Select one</option>
            {COMFORT_OPTIONS.map(([value, label]) => (
          ...
          {errors.comfortLevel && (
            <p id="interest-comfort-error" className={errorClass}>
              {errors.comfortLevel}
            </p>
          )}
        </div>
      ...
      <div className="grid gap-5">
        <TextAreaField
          ...
          onChange={(value) => update("personalMission", value)}
          maxLength={4000}
        />
        <TextAreaField
          id="interest-fundraising"
          label="Fundraising interest or goal (optional)"
          value={form.fundraisingGoal}
          onChange={(value) => update("fundraisingGoal", value)}
          maxLength={1000}
        />
        <TextAreaField
        ...
        />
        <p className="text-sm text-muted-foreground">
          Please do not include medical records, Social Security numbers, VA
          claim evidence, or other sensitive documents.
        </p>
      </div>

      <fieldset
        className="space-y-4 rounded-lg border border-border bg-muted/20 p-4 sm:p-5"
        aria-describedby={errors.socials ? "interest-socials-error" : undefined}
      >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-muted-foreground">
              Add up to 10 useful profiles. Follower count is optional.
            </p>
          </div>
          <button
            type="button"
            disabled={form.socials.length >= 10}
            onClick={() => update("socials", [...form.socials, newSocial()])}
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-border bg-background px-4 py-2 text-sm font-medium hover:bg-muted disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Plus className="h-4 w-4" aria-hidden /> Add social profile
          </button>
        </div>
        {errors.socials && (
          <p id="interest-socials-error" className={errorClass}>
            {errors.socials}
          </p>
        )}
          ...
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <button
                ...
                onClick={() =>
                  update(
                    "socials",
                    form.socials.filter((item) => item.id !== social.id),
                  )
                }
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  aria-describedby={
                    errors[`socials.${index}.platform`]
                      ? `social-platform-${social.id}-error`
                      : undefined
                  }
                  onChange={(event) =>
                ...
                {errors[`socials.${index}.platform`] && (
                  <p
                    id={`social-platform-${social.id}-error`}
              ...
              <Field
                ...
                value={social.handle}
                onChange={(value) => updateSocial(social.id, "handle", value)}
                maxLength={200}
              />
              <Field
                ...
                error={
                  errors[`socials.${index}.profileUrl`] ??
                  errors[`socials.${index}.account`]
                }
                onChange={(value) =>
                  updateSocial(social.id, "profileUrl", value)
                }
                maxLength={500}
              />
              <Field
                id={`social-followers-${social.id}`}
                label="Follower count"
                type="number"
                value={social.followerCount}
                error={errors[`socials.${index}.followerCount`]}
                onChange={(value) =>
                  updateSocial(social.id, "followerCount", value)
                }
                min={0}
                max={2147483647}
              />
      ...
      <label className="flex items-start gap-3 rounded-lg border border-border bg-muted/30 p-4 text-sm text-foreground">
        <input
          ...
          aria-describedby={
            errors.consent ? "interest-consent-error" : undefined
          }
          onChange={(event) => update("consent", event.target.checked)}
          className="mt-0.5 h-4 w-4"
        />
        <span>
          I consent to ValorWell contacting me about this interest submission.{" "}
          <span aria-hidden className="text-destructive">
            *
          </span>
        </span>
      ...
      {errors.consent && (
        <p id="interest-consent-error" className={errorClass}>
          {errors.consent}
        </p>
      )}
      ...
      <button
        ...
        className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-[hsl(var(--navy))] px-6 py-3 text-sm font-semibold text-white hover:bg-[hsl(var(--navy-light))] disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
      >
        {status === "loading" ? (
          <>
          ...
          <>
            Send my interest <ArrowRight className="h-4 w-4" aria-hidden />
          </>
        )}
      </button>
    </form>
