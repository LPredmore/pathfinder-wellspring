...
const lanes = [
  {
    value: "veteran",
    label: "Veteran seeking help with a mental-health claim or appeal",
  },
  {
    value: "partnership_support",
    label: "Organization, supporter, creator, sponsor, or connector",
  },
  {
    value: "general",
    label: "I want to help, but I am not sure where I fit",
  },
] as const;
...
export function UnifiedOcsRoutingForm() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    trackHomeEvent("ocs_form_submit", {
      page: "operation-claims-success",
      lane: form.lane,
    });
    setStatus("success");
  };
  ...
  if (status === "success") {
    return (
      <div className="rounded-2xl border border-[#3B5147]/20 bg-white p-8 text-center shadow-sm">
        ...
        <h3 className="text-2xl font-bold text-[#111814]">
          Your information has been received.
        </h3>
        <p className="mx-auto mt-3 max-w-xl text-[#111814]/70">
          ValorWell will review your state, situation, and selected path and reach out
          within 48 hours with the appropriate next steps.
        </p>
      </div>
    );
  }
  ...
  return (
    <form
      onSubmit={submit}
      className="space-y-8 rounded-2xl border border-[#3B5147]/20 bg-white p-6 shadow-sm md:p-8"
    >
      <div className="rounded-xl border border-[#3B5147]/20 bg-[#F4F1E8] p-4 text-sm text-[#111814]/75">
        <span className="font-semibold text-[#111814]">Mental-health clinicians:</span>{" "}
        provider participation and recruiting now live on the{" "}
      ...
      <fieldset>
        <legend className="text-lg font-semibold text-[#111814]">
          1. Choose the closest path
        </legend>
        <p className="mt-1 text-sm text-[#111814]/70">
          This is a routing form, not a clinical intake. Do not include medical
          records, VA claim files, diagnoses, Social Security numbers, or detailed
          trauma information.
        </p>
      ...
      {form.lane && (
        <>
          <fieldset className="grid gap-4 md:grid-cols-2">
            <legend className="col-span-full text-lg font-semibold text-[#111814]">
              2. Contact details
            </legend>
            <Field
              label="First name"
          ...
          {form.lane === "veteran" && (
            <fieldset className="grid gap-4 md:grid-cols-2">
              <legend className="col-span-full text-lg font-semibold text-[#111814]">
                3. Help us check your current path
              </legend>
              <SelectField
                label="Relationship to the veteran"
                required
                value={form.relationshipToVeteran}
                options={["Veteran", "Active duty", "Family member", "Caregiver", "Other"]}
                onChange={(value) => update("relationshipToVeteran", value)}
              />
              <Field
                label="Veteran's state"
                required
                value={form.state}
                onChange={(value) => update("state", value)}
              />
              <SelectField
                label="Current claim or appeal stage"
                value={form.claimStage}
                options={[
                  "Considering a new claim",
                  "Preparing an original claim",
                  "Requesting an increased rating",
                  "Exploring a secondary condition",
                  "Responding to a denial",
                  "Appeal in progress",
                  "Not sure",
                ]}
                onChange={(value) => update("claimStage", value)}
              />
              <SelectField
                label="Current mental-health care"
                value={form.currentCare}
                options={[
                  "No current care",
                  "Receiving care through the VA",
                  "Receiving care outside the VA",
                  "Previously received care",
                  "Not sure",
                ]}
                onChange={(value) => update("currentCare", value)}
              />
            </fieldset>
          )}
          ...
          {form.lane === "partnership_support" && (
            <fieldset className="space-y-5 rounded-lg border border-[#3B5147]/20 bg-[#F4F1E8]/50 p-4">
              <legend className="px-1 text-lg font-semibold text-[#111814]">
                3. Relationship classification
              </legend>
              <p className="text-sm text-[#111814]/70">
                Select every description that applies.
              </p>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {relationshipOptions.map(([value, label]) => (
                  <label
                    key={value}
                    className="flex items-start gap-2 rounded-md border border-[#3B5147]/15 bg-white p-3 text-sm"
                  >
                    <input
                      type="checkbox"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field
                  ...
                  onChange={(value) => update("socialLink", value)}
                />
                <SelectField
                  ...
                  onChange={(value) => update("organizationKind", value)}
                />
                <label className="flex items-center gap-3 rounded-md border border-[#3B5147]/15 bg-white p-3 text-sm">
                  <input
                    ...
                    onChange={(event) =>
                      update("veteranAffiliated", event.target.checked)
                    }
          ...
          <div>
            <label
              htmlFor="ocs_details"
              className="block text-sm font-medium text-[#111814]"
            >
              {form.lane === "veteran"
          ...
          <label className="flex items-start gap-3 text-sm text-[#111814]">
            ...
            />
            <span>
              I understand this submission does not guarantee immediate clinical care,
              VA authorization, documentation, partnership, or any VA outcome.
            </span>
          </label>
          ...
          {status === "error" && (
            <div
              role="alert"
              className="rounded-md border border-[#B24A3A]/40 bg-[#B24A3A]/5 p-3 text-sm text-[#B24A3A]"
            >
              {errorMessage}
            </div>
          )}
