...
import { buildCreatorInterestPayload, UnifiedBtyForm, type CreatorInterestFormState } from "./UnifiedBtyForm";
...
function fillRequired(email = "Person@Example.com") {
  ...
  fireEvent.change(screen.getByLabelText(/Why are you interested/), { target: { value: "I want to help tell useful stories." } });
  fireEvent.change(screen.getByLabelText(/How would you like to participate/), { target: { value: "I can create educational community content." } });
  fireEvent.change(screen.getByLabelText(/Are you willing to share/), { target: { value: "not_sure" } });
  fireEvent.change(screen.getByLabelText(/Participation comfort level/), { target: { value: "flexible" } });
...
describe("UnifiedBtyForm", () => {
  ...
  it("builds only the public RPC whitelist and omits empty optional fields", () => {
    ...
    expect(Object.keys(payload).sort()).toEqual([
      "comfort_level", "consent", "email", "first_name", "last_name", "motivation", "participation",
  ...
  it("shows client-side validation and does not call the RPC", () => {
    render(<UnifiedBtyForm />);
    fireEvent.submit(screen.getByRole("button", { name: /Send my interest/ }).closest("form")!);
  ...
  it("rejects an aggregate UTF-8 payload before calling the RPC", () => {
    ...
    fireEvent.change(screen.getByLabelText(/Why are you interested/), { target: { value: text1900 } });
    fireEvent.change(screen.getByLabelText(/How would you like to participate/), { target: { value: text1900 } });
    fireEvent.change(screen.getByLabelText(/Personal mission/), { target: { value: text1900 } });
    fireEvent.change(screen.getByLabelText(/Anything else/), { target: { value: "🚀".repeat(3900) } });
    fireEvent.click(screen.getByRole("button", { name: /Send my interest/ }));
    expect(screen.getByRole("alert")).toHaveTextContent(/review the highlighted fields/i);
    expect(mocks.rpc).not.toHaveBeenCalled();
  });
  ...
  it("adds several social rows, removes one, and submits only the remaining normalized row", async () => {
    ...
    fireEvent.change(screen.getAllByLabelText(/Follower count/)[1], { target: { value: "1200" } });
    fireEvent.click(screen.getByRole("button", { name: "Remove social profile 1" }));
    fireEvent.click(screen.getByRole("button", { name: /Send my interest/ }));
    ...
    expect(mocks.rpc).toHaveBeenCalledWith("submit_website_creator_interest", {
      p_payload: expect.objectContaining({
        ...
        social_profiles: [{ platform: "YouTube", handle: "@keep-me", follower_count: 1200 }],
      }),
    });
  });

  it("shows a safe generic failure without exposing backend details", async () => {
    mocks.rpc.mockResolvedValueOnce({ data: null, error: { message: "relation secret_table does not exist" } });
    render(<UnifiedBtyForm />);
    fillRequired();
    fireEvent.click(screen.getByRole("button", { name: /Send my interest/ }));
    expect(await screen.findByRole("alert")).toHaveTextContent("We could not submit the form right now.");
  ...
  it("submits the same normalized email twice with separate idempotency keys", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Send my interest/ }));
    expect(await screen.findByRole("status")).toHaveTextContent(/received your interest/i);
    first.unmount();
